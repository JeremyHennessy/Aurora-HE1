...
from aurora_he1.aero import aero_point  # noqa: E402
from aurora_he1.config import load_config  # noqa: E402
from aurora_he1.geometry import solve_trapezoidal_wing  # noqa: E402
from aurora_he1.model import evaluate  # noqa: E402
from aurora_he1.propulsion import cruise_power_split  # noqa: E402
...
def main() -> int:
    ...
    with (out / "airfoil_stations.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["eta", "semi_span_y_m", "chord_m", "airfoil", "twist_deg", "reynolds"], lineterminator="\n")
        writer.writeheader()
        writer.writerows(result["airfoil_stations"])

    with (out / "mass_budget.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["name", "mass_kg", "x_m"], lineterminator="\n")
        writer.writeheader()
        writer.writerows(cfg["mass"]["items"])

    env = cfg["environment"]
    ac = cfg["aero"]
    pc = cfg["propulsion"]
    wing = solve_trapezoidal_wing(
        cfg["geometry"]["wing"]["span_m"],
        cfg["geometry"]["wing"]["area_m2"],
        cfg["geometry"]["wing"]["taper_ratio"],
    )
