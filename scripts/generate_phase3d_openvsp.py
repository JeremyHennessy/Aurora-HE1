...
def script_for_case(cfg: dict, wing: dict, tail: dict, case: dict) -> str:
    ...
    L += [
        ...
        '    string eta_flag_id=""; string eta_start_id=""; string eta_end_id=""; string absrel_id=""; string const_id=""; string lcs_id=""; string lce_id="";',
        ...
        '    for (uint i=0; i<uint(pids.size()); i++) { string n=GetParmName(pids[i]); if(n=="EtaFlag"){eta_flag_id=pids[i];found_eta_flag=1;} else if(n=="EtaStart"){eta_start_id=pids[i];found_eta_start=1;} else if(n=="EtaEnd"){eta_end_id=pids[i];found_eta_end=1;} else if(n=="Abs_Rel_Flag"){absrel_id=pids[i];found_absrel=1;} else if(n=="SE_Const_Flag"){const_id=pids[i];found_const=1;} else if(n=="Length_C_Start"){lcs_id=pids[i];found_lcs=1;} else if(n=="Length_C_End"){lce_id=pids[i];found_lce=1;} }',
        ...
        f'    if (!(found_eta_flag==1 && found_eta_start==1 && found_eta_end==1 && found_absrel==1 && found_const==1 && found_lcs==1 && found_lce==1)) {{ Print("PHASE3D_ERROR|{cid}|control surface parameter contract incomplete"); return; }}',
        '    SetParmVal(eta_flag_id, 1.0);',
        f'    SetParmVal(eta_start_id, {eta_start:.12f}); SetParmVal(eta_end_id, {eta_end:.12f});',
        '    SetParmVal(absrel_id, 1.0); SetParmVal(const_id, 1.0);',
        f'    SetParmVal(lcs_id, {chord_fraction:.12f}); SetParmVal(lce_id, {chord_fraction:.12f});',
        '    Update();',
        '    double actual_eta_start=GetParmVal(eta_start_id); double actual_eta_end=GetParmVal(eta_end_id); double actual_lcs=GetParmVal(lcs_id); double actual_lce=GetParmVal(lce_id);',
        f'    Print("PHASE3D_CONTROL_GEOM|{cid}|{eta_start:.12f}|{eta_end:.12f}|",false); Print(actual_eta_start,false); Print("|",false); Print(actual_eta_end,false); Print("|",false); Print(actual_lcs,false); Print("|",false); Print(actual_lce);',
        f'    if (actual_eta_start < {eta_start - 1e-8:.12f} || actual_eta_start > {eta_start + 1e-8:.12f} || actual_eta_end < {eta_end - 1e-8:.12f} || actual_eta_end > {eta_end + 1e-8:.12f} || actual_lcs < {chord_fraction - 1e-8:.12f} || actual_lcs > {chord_fraction + 1e-8:.12f} || actual_lce < {chord_fraction - 1e-8:.12f} || actual_lce > {chord_fraction + 1e-8:.12f}) {{ Print("PHASE3D_ERROR|{cid}|control geometry readback mismatch"); return; }}',
        '    AutoGroupVSPAEROControlSurfaces(); Update();',
